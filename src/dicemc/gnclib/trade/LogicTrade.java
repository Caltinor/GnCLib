...
public class LogicTrade implements IDBImplTrade{
	...
	public TranslatableResult<ResultType> createTransaction(IMarketEntry entry, MarketType type) {
		...
		switch (type) {
		case LOCAL: {
			if (!(entry instanceof EntryLocal)) return new TranslatableResult<ResultType>(ResultType.FAILURE, "lib.market.create.failure.typemismatch");
			if (!entry.getGiveItem()) {
				double balP = LogicMoney.getBalance(entry.getVendor().refID, LogicMoney.agentType(entry.getVendor().type));
				if (balP < entry.getPrice() * entry.getStock()) return new TranslatableResult<ResultType>(ResultType.FAILURE, "lib.market.create.failure.funds");
				LogicMoney.changeBalance(entry.getVendor().refID, LogicMoney.agentType(entry.getVendor().type), -(entry.getPrice() * entry.getStock()));
			}
			break;
		}
		case GLOBAL: {
			if (!(entry instanceof EntryGlobal)) return new TranslatableResult<ResultType>(ResultType.FAILURE, "lib.market.create.failure.typemismatch");
			...
			if (pBal < fee) {return new TranslatableResult<ResultType>(ResultType.FAILURE, "lib.market.create.failure.funds");}
			else {LogicMoney.changeBalance(entry.getVendor().refID, LogicMoney.agentType(entry.getVendor().type), -(fee));}
			break;
		}
		case AUCTION: {
			if (!(entry instanceof EntryAuction)) return new TranslatableResult<ResultType>(ResultType.FAILURE, "lib.market.create.failure.typemismatch");
			double pBal = LogicMoney.getBalance(entry.getVendor().refID, LogicMoney.agentType(entry.getVendor().type));
	...
	public TranslatableResult<ResultType> acceptOffer(IMarketEntry entry, MarketType type, EntryOffer offer) {
		...
		case SERVER: case AUCTION: {
			return new TranslatableResult<ResultType>(ResultType.FAILURE, "lib.market.acceptoffer.failure.type");}
		case LOCAL: case GLOBAL: {
			if (offer.requestedAmount > entry.getStock()) return new TranslatableResult<ResultType>(ResultType.FAILURE, "lib.market.acceptoffer.failure.count");
			break;
		}		
		default:}
	...
	public TranslatableResult<ResultType> executeTransaction(IMarketEntry entry, MarketType type, Agent buyer, int count) {
		if (type.equals(MarketType.AUCTION)) return new TranslatableResult<ResultType>(ResultType.FAILURE, "lib.market.executetrans.failue.type");
		if (isStockInsufficient(entry.getStock(), count)) return new TranslatableResult<ResultType>(ResultType.FAILURE, "lib.market.executetrans.failure.count");
		...
		if (balP < (cost + fee)) return new TranslatableResult<ResultType>(ResultType.FAILURE, "lib.market.executetrans.failure.fund");
	...
	public TranslatableResult<ResultType> submitOffer(IMarketEntry entry, EntryOffer offer, MarketType type) {
		if (type.equals(MarketType.AUCTION) || type.equals(MarketType.SERVER) || !entry.getGiveItem()) {
			return new TranslatableResult<ResultType>(ResultType.FAILURE, "lib.market.offer.failure.type");
		}
		else if (entry.getStock() < offer.requestedAmount) {
			return new TranslatableResult<ResultType>(ResultType.FAILURE, "lib.market.offer.failure.stock");
		}
			...
			if (balP < fee) return new TranslatableResult<ResultType>(ResultType.FAILURE, "lib.market.offer.failure.fund");
			LogicMoney.changeBalance(offer.offerer.refID, LogicMoney.agentType(offer.offerer.type), -(fee));
		}
		else return new TranslatableResult<ResultType>(ResultType.FAILURE, "");
	...
	public TranslatableResult<ResultType> placeBid(EntryBid bid, double itemValue) {
		...
		if (bidList.size() > 0) {
			...
			if (highestBid.value >= bid.value) {return new TranslatableResult<ResultType>(ResultType.FAILURE, "lib.market.placebid.failure.toolow");}
		}
		else { 
			if (itemValue >= bid.value) {return new TranslatableResult<ResultType>(ResultType.FAILURE, "lib.market.placebid.failure.toolow");}
		}
		double pBal = LogicMoney.getBalance(bid.bidder.refID, LogicMoney.agentType(bid.bidder.type));
		if (pBal < bid.value) {return new TranslatableResult<ResultType>(ResultType.FAILURE, "lib.market.placebid.failure.fund");}
	...
	public TranslatableResult<ResultType> pullFromStorage(EntryStorage entry, int count) {
		...
		if (count > entry.count) return new TranslatableResult<ResultType>(ResultType.FAILURE, "lib.market.storage.pull.failure.count");
	...
	public TranslatableResult<ResultType> changeTransactionSupply(MarketType type, IMarketEntry entry, int newSupply) {
		if (type.equals(MarketType.AUCTION)) return new TranslatableResult<ResultType>(ResultType.FAILURE, "lib.market.supplychange.failure.type");
		if ((entry.getStock() + newSupply) < 0) return new TranslatableResult<ResultType>(ResultType.FAILURE, "lib.market.supplychange.failure.negative");
		...
		if (Marketplaces.get(type).sellFee > 0) {
			...
			double balP = LogicMoney.getBalance(entry.getVendor().refID, LogicMoney.agentType(entry.getVendor().type));
			if (fee > balP) return new TranslatableResult<ResultType>(ResultType.FAILURE, "lib.market.supplychange.failure.fund");
